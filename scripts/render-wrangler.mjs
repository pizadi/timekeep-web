...
const need = (name) => {
	...
	if (!v || v.trim() === '') {
		console.error(`render-wrangler: env ${name} is required (GitHub repo variable CF_${name.replace('DATABASE_ID', 'DATABASE_ID').replace('NAMESPACE_ID', 'NAMESPACE_ID')} → check the deploy workflow's mapping)`);
		process.exit(1);
	}
	return v.trim();
};
