...
async function main() {
  ...
  assert(
    p1.status === 200 && Array.isArray(p1.body.sessions) && p1.body.sessions.length <= 5,
    `page 1 returns ≤5 rows (${p1.body.sessions?.length})`,
  );
  assert(
    Number.isInteger(p1.body.total) && p1.body.total >= p1.body.sessions.length,
  ...
  if (p1.body.total > 5) {
    ...
    assert(
      p2.body.sessions.every((s) => !ids1.has(s.id)),
      'pages are disjoint',
    );
  } else {
  ...
  assert(
    day.status === 200 && /^\d{4}-\d{2}-\d{2}$/.test(day.body.day),
    `day summary defaults to today (${day.body.day})`,
  );
  assert(
    Number.isInteger(day.body.total_minutes) && Array.isArray(day.body.projects) && Array.isArray(day.body.tasks),
  ...
  if (picked) {
    ...
    assert(
      Math.abs(sum - picked.total_minutes) <= 1,
    ...
    assert(
      Math.abs(total - day.body.total_minutes) <= day.body.projects.length,
  ...
  assert(
    sum.body.donut_subtasks.every((d) => typeof d.task_name === 'string' && Number.isInteger(d.minutes)),
