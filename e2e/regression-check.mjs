...
async function login(identifier, password) {
  ...
  const csrf = (res.headers.get('set-cookie') ?? '')
    .split('set-cookie:').map(() => '').join(''); // csrf read below via /config-free call
...
const ch = await raw('/me/password', { method: 'POST', body: { current_password: 'regression-pass-42', password: 'regression-new-pass-42' } }, u.cookie);
...
const open = await raw('/sessions', { method: 'POST', body: { task_id: task.id, started_at: Date.now() - 60_000, ended_at: null } }, ck);
check('open-ended manual session rejected with 422', open.status === 422, `status ${open.status}`);
const openPatch = await raw('/sessions', { method: 'POST', body: { task_id: task.id, started_at: Date.now(), ended_at: Date.now() + 120_000 } }, ck);
check('closed manual session still accepted', openPatch.status === 201, `status ${openPatch.status} ${JSON.stringify(openPatch.body).slice(0, 120)}`);
...
check('timer start after deleting the running task → 200 (no ghost 409)', nextStart.status === 200, `status ${nextStart.status} ${JSON.stringify(nextStart.body).slice(0, 120)}`);
...
check('reset-request response carries no reset link', rr.status === 200 && rr.body.dev_reset_url === undefined && !JSON.stringify(rr.body).includes('token'), JSON.stringify(rr.body));
...
check('resend-verification reports no-mailbox accounts (422 no_email)', rvAuthed.status === 422 && rvAuthed.body.error?.code === 'no_email', `status ${rvAuthed.status}`);
...
await raw('/admin/users', { method: 'POST', body: { username: withMail, name: 'WM', email: `withmail${wmId}@example.com`, password: 'withmail-pass-424242' } }, CK);
const wmLogin = await login(withMail, 'withmail-pass-424242');
const wmChange = await raw('/me/password', { method: 'POST', body: { current_password: 'withmail-pass-424242', password: 'withmail-new-pass-424242' } }, wmLogin.cookie);
...
check('resend-verification OK for pre-verified mailbox user', rvMail.status === 200 && rvMail.body.ok === true, `status ${rvMail.status}`);
...
const imp = await raw('/import', {
  method: 'POST',
  body: {
    mode: 'merge',
    data: {
      projects: [{ id: pA.id, name: pA.name }],
      tasks: [{ id: iA.id, project_id: pA.id, name: 'a' }, { id: iB.id, project_id: pA.id, name: 'b' }],
...
check('cyclic import edge skipped', imp.status === 200 && imp.body.summary.dependencies.created === 0, JSON.stringify(imp.body.summary?.dependencies));
check('epoch-1970/null-start session skipped', imp.status === 200 && imp.body.summary.sessions.created === 0, JSON.stringify(imp.body.summary?.sessions));
check('import.completed event emitted', (imp.body.events ?? []).some((e) => e.type === 'import.completed'));

// --- import with entity-limit overflow → 422 ---
const impLimit = await raw('/import', {
  method: 'POST',
  body: { mode: 'duplicate', data: { projects: Array.from({ length: 250 }, (_, i) => ({ id: `01ARZ3NDEKTSV4RRFFQ69G5F${String(i).padStart(2, '0')}`, name: `p${i}` })) } }
}, ck);
...
await raw('/me/password', { method: 'POST', body: { current_password: 'foreign-user-pass-42', password: 'foreign-user-new-pass-42' } }, otherLogin.cookie);
...
const otherTask = (await raw(`/projects/${otherProj.id}/tasks`, { method: 'POST', body: { name: 'other-t' } }, otherCk)).body.task;
check('fixture (other user task exists)', !!otherTask);
const foreignRestore = await raw('/restore', {
  method: 'POST',
  body: { dependencies: [{ task_id: task2.id, depends_on_id: otherTask.id }] }
}, ck);
check('cross-user dependency row rejected', foreignRestore.status === 200 && foreignRestore.body.restored === 0, JSON.stringify(foreignRestore.body));
