...
async function adminSession() {
  ...
  if (r.status !== 200) {
    ...
    if (r.status !== 200 || !r.data?.must_change_password) throw new Error(`admin bootstrap failed: ${JSON.stringify(r.data)}`);
    const ch = await raw(s, '/me/password', { method: 'POST', body: { current_password: 'changemeasap', password: adminPw } });
...
async function main() {
  ...
  const proj = must(await raw(victim, '/projects', { method: 'POST', origin: BASE, body: { name: `SecretProj${stamp}` } }), 'victim project').project;
  const task = must(await raw(victim, `/projects/${proj.id}/tasks`, { method: 'POST', origin: BASE, body: { name: 'SecretTask' } }), 'victim task').task;
  const now = Date.now();
  const sess = must(await raw(victim, '/sessions', {
    method: 'POST', origin: BASE,
    body: { task_id: task.id, started_at: now + 60_000, ended_at: now + 120_000, note: `=cmd|' /C calc'!A0 secret-note` }
  }), 'victim session').session;

  // ---------- 1. unauthenticated matrix ----------
  const unauth = jar();
  for (const [m, p] of [
    ['GET', '/bootstrap'], ['GET', '/me'], ['GET', '/projects'], ['GET', '/me/sessions'],
    ['GET', '/sync'], ['GET', '/reports/summary'], ['GET', '/export'],
    ['POST', '/projects'], ['PATCH', '/me'], ['DELETE', '/me'],
    ['POST', '/timer/start'], ['POST', '/pomo/skip'], ['POST', '/admin/users'], ['GET', '/admin/users'],
    ['PATCH', '/subtasks/01ARZ3NDEKTSV4RRFFQ69G5FAV'], ['DELETE', '/subtasks/01ARZ3NDEKTSV4RRFFQ69G5FAV'],
    ['PATCH', '/tasks/01ARZ3NDEKTSV4RRFFQ69G5FAV'], ['DELETE', '/tasks/01ARZ3NDEKTSV4RRFFQ69G5FAV']
  ]) {
    const r = await raw(unauth, p, { method: m, body: m !== 'GET' ? {} : undefined });
    check(`unauth ${m} ${p.split('?')[0]} → 401`, r.status === 401, `got ${r.status}`);
  }
  {
    const su = await raw(unauth, '/auth/signup', { method: 'POST', body: { username: 'x', password: 'nope-nope-nope-99' } });
    check('unauth /auth/signup → 404', su.status === 404, `got ${su.status}`);
  }

  // ---------- 2. cross-account authz (IDOR) ----------
  const task2 = (await raw(victim, `/projects/${proj.id}/tasks`, { method: 'POST', origin: BASE, body: { name: 'SecretTask2' } })).data.task;
  for (const [m, p, body] of [
    ['GET', `/projects/${proj.id}`], ['PATCH', `/projects/${proj.id}`, { name: 'hacked' }],
    ['DELETE', `/projects/${proj.id}`], ['GET', `/projects/${proj.id}/tasks`],
    ['PATCH', `/tasks/${task.id}`, { name: 'hacked' }], ['DELETE', `/tasks/${task.id}`],
    ['GET', `/sessions/${sess.id}`], ['DELETE', `/sessions/${sess.id}`],
  ...
  }
  {
    ...
    check('IDOR GET layout leaks nothing (empty own rows)', gl.status === 200 && (gl.data?.positions ?? []).length === 0, `got ${gl.status} ${(gl.data?.positions ?? []).length} rows`);
    const pl = await raw(attacker, `/layout/${proj.id}`, { method: 'PUT', origin: BASE, body: { positions: [{ task_id: task.id, x: 1, y: 1 }] } });
    const vl = await raw(victim, `/layout/${proj.id}`);
    check('IDOR PUT layout cannot write via foreign project', pl.status === 200 && (vl.data?.positions ?? []).length === 0, `victim rows: ${(vl.data?.positions ?? []).length}`);
  }
  {
    ...
    const exLeak = JSON.stringify(ex.data).includes(`SecretProj${stamp}`) || JSON.stringify(ex.data).includes('secret-note');
  ...
  // ---------- 3. timer/pomo task ownership (DO path) ----------
  {
    ...
    check('IDOR timer/start on foreign task rejected', r.status === 404, `got ${r.status} ${JSON.stringify(r.data).slice(0, 80)}`);
  ...
  // ---------- 4. task → foreign project move ----------
  {
    ...
    const ownTask = (await raw(attacker, `/projects/${ownProject.id}/tasks`, { method: 'POST', origin: BASE, body: { name: 'Mover' } })).data.task;
    const r = await raw(attacker, `/tasks/${ownTask.id}`, { method: 'PATCH', origin: BASE, body: { project_id: foreignProject.id } });
  ...
  // ---------- 6. forced password change gate ----------
  {
    ...
    await raw(admin, '/admin/users', { method: 'POST', origin: BASE, body: { username: u, password: `audit-flag-${stamp}-pw` } });
    ...
    const t = (li.headers.getSetCookie?.() ?? []).find((c) => c.startsWith('tk_csrf='))?.split(';')[0]?.split('=')[1];
    for (const [m, p, body] of [
      ['GET', '/bootstrap'], ['GET', '/projects'], ['GET', '/sync'], ['GET', '/reports/summary'],
      ['GET', '/export'], ['GET', '/admin/users'], ['POST', '/timer/start'], ['PATCH', '/me'], ['DELETE', '/me']
    ]) {
      const r = await raw(flagged, p, { method: m, body, origin: BASE, csrf: t });
      check(`gate ${m} ${p} → 403 password_change_required`, r.status === 403 && r.data?.error?.code === 'password_change_required', `got ${r.status}`);
    }
    const me = await raw(flagged, '/me');
    check('gate GET /me stays readable', me.status === 200);
    const ch = await raw(flagged, '/me/password', { method: 'POST', origin: BASE, csrf: t, body: { current_password: `audit-flag-${stamp}-pw`, password: `audit-flag-${stamp}-done` } });
  ...
  {
    const s1 = jar(); const s2 = jar();
    const u = `aud_rev_${stamp}_${Math.floor(Math.random() * 1000)}`;
    const revPw = `audit-rev-${stamp}-pw`;
    const cr = await raw(admin, '/admin/users', { method: 'POST', origin: BASE, body: { username: u, password: revPw } });
    ...
    const d1 = await raw(s1, '/me'); const d2 = await raw(s2, '/me');
    ...
    check('deactivated login → 403 account_disabled', rlog.status === 403 && rlog.data?.error?.code === 'account_disabled', `got ${rlog.status}`);
    // re-activate + admin password reset → revoked + flagged
    await raw(admin, `/admin/users/${uid}`, { method: 'PATCH', origin: BASE, body: { active: 1 } });
    await raw(admin, `/admin/users/${uid}/password`, { method: 'POST', origin: BASE, body: { password: `${revPw}-np` } });
  ...
  // ---------- 8. response field whitelists (sensitive data) ----------
  {
    ...
    check('/me exposes no sensitive fields', badMe.every((k) => !meKeys.includes(k)), meKeys.join(','));
    const b = await raw(s, '/bootstrap');
    const bootKeys = Object.keys(b.data.user ?? {});
    check('/bootstrap user exposes no sensitive fields', badMe.filter((k) => k !== 'active').every((k) => !bootKeys.includes(k)), bootKeys.join(','));
    const lg = await raw(s, '/auth/login', { method: 'POST', body: { identifier: `aud_b_${stamp}`, password: victimPw } });
    const loginKeys = Object.keys(lg.data ?? {});
    check('login response minimal', loginKeys.every((k) => ['ok', 'user_id', 'role', 'must_change_password'].includes(k)), loginKeys.join(','));
    const sessions = await raw(s, '/me/sessions');
    const sessKeys = Object.keys(sessions.data.sessions?.[0] ?? {});
    check('/me/sessions exposes only session fields', sessKeys.every((k) => ['id', 'user_agent', 'ip', 'created_at', 'last_seen_at', 'expires_at', 'current'].includes(k)), sessKeys.join(','));
  }
  ...
  // the login limits, which do 429 in e2e).
  {
    ...
    else console.log(`SKIP  heavy-API rate limit probe (needs RL_API_USER=10 in .dev.vars to exceed locally; all codes: ${[...codes].join(',')})`);
  }

  // ---------- 10. WS upgrade without session ----------
  {
    const res = await fetch(`${BASE.replace('http', 'ws')}/api/ws?device=audit`, { headers: { upgrade: 'websocket' } }).catch((e) => ({ status: 0, error: e.message }));
    check('WS upgrade without session rejected', res.status === 401 || res.status === 0, `got ${res.status ?? res.error}`);
  }
...
main().catch((e) => { console.error('PROBES CRASHED:', e.message); process.exit(1); });
