...
async function main() {
  ...
  const task = (await raw(`/projects/${proj.id}/tasks`, { method: 'POST', body: { name: `UndoMe-${Date.now() % 10000}` } }, ck)).body.task;
  const sb = (await raw(`/tasks/${task.id}/subtasks`, { method: 'POST', body: { name: 'checklist item' } }, ck)).body.subtask;
  ...
  const now = Date.now();
  const sessRes = await raw('/sessions', {
    method: 'POST', body: { task_id: task.id, started_at: now + 60_000, ended_at: now + 240_000, note: 'undo me' }
  }, ck);
  if (sessRes.status !== 201) { console.error('fixture session create failed:', JSON.stringify(sessRes.body)); process.exit(1); }
  ...
  console.log(`after undo — task:${!!t2} subtask(id kept):${!!sb2} dep:${!!dep2} session(note matched):${log.length === 1}`);
