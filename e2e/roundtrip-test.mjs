...
async function main() {
  ...
  const a = cookieOf(await raw('/auth/login', { method: 'POST', body: { identifier: 'dana', password: 'purple-marmalade-tuesday' } }));
  const exported = await fetch(`${BASE}/api/export?format=json`, { headers: { cookie: a } }).then((r) => r.json());
  console.log(`exported: projects=${exported.projects.length} tasks=${exported.tasks.length} subtasks=${exported.subtasks.length} deps=${exported.dependencies.length} sessions=${exported.sessions.length}`);
  ...
  const createB = await raw('/admin/users', {
    method: 'POST',
    body: { username: bName, name: 'RT', password: 'rt-temp-passphrase-42' }
  }, admin);
  if (createB.status !== 201) { console.error('create user B failed', createB.body); process.exit(1); }
  const bLogin = await raw('/auth/login', { method: 'POST', body: { identifier: bName, password: 'rt-temp-passphrase-42' } });
  const b = cookieOf(bLogin);
  if (!b || !bLogin.body?.must_change_password) { console.error('user B login failed', bLogin.body); process.exit(1); }
  const chB = await raw('/me/password', {
    method: 'POST',
    body: { current_password: 'rt-temp-passphrase-42', password: 'another-secure-passphrase-42' }
  }, b);
  if (chB.status !== 200) { console.error('user B password change failed', chB.body); process.exit(1); }
  const aLoginGone = await raw('/auth/login', { method: 'POST', body: { identifier: 'dana', password: 'purple-marmalade-tuesday' } });
  ...
  const sameIds = (arrA, arrB) =>
    arrA.length === arrB.length && arrA.every((x) => arrB.some((y) => y.id === x.id));
  ...
  const sessionsOk = logB.body.sessions.length === exported.sessions.filter((s) => s.ended_at).length + Math.min(exported.sessions.filter((s) => !s.ended_at).length, 200);
  console.log(`ids preserved — tasks:${tasksOk} deps:${depsOk} subtaskStates:${subOk} sessionsImported:${logB.body.sessions.length}/${exported.sessions.length}`);
  ...
  console.log(`pomodoro — start→focus:${phase1 === 'focus'} timerStartedWithPomo:${running} skip→idle:${phase2 === 'idle'}`);
