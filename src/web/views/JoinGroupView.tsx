...
interface Preview { group: { id: string; name: string; color: string; member_count: number } }
...
export default function JoinGroupView({ token, onClose }: { token: string; onClose?: () => void }) {
  ...
  return (
    <div className="content" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 40 }}>
      <div className="card" style={{ maxWidth: 460, width: '100%' }}>
        <h3>{error ? 'Invite unavailable' : 'Join group'}</h3>
        {error ? (
          <>
            <p className="muted">{error}.</p>
            <button className="btn primary" onClick={() => { onClose?.(); go(pathForView(store.get().view)); }}>Go to app</button>
          </>
        ) : !preview ? (
          ...
          <>
            <p>
              ...
              <span className="muted"> · {preview.group.member_count} member{preview.group.member_count === 1 ? '' : 's'}</span>
            </p>
            <p className="muted">Joining shows you the group's chat and shared projects; members see your membership and group activity.</p>
