    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 760, margin: '0 auto', width: '100%' }}>
      <div className="card">
        ...
        <p className="muted" style={{ marginTop: 0 }}>
          A group shares a member list, a chat and group projects. Invite by username or a
          shareable link; the owner grants admins exactly the permissions you pick. Chats open
          in docked windows at the bottom-right of the screen.
        </p>
              ...
              <div key={g.id}>
                <div className="row" role="button" tabIndex={0} aria-expanded={expanded === g.id}
                  ...
                  <span className="grow">
                    ...
                    {g.unread > 0 && <span className="muted"> · <b style={{ color: 'var(--accent)' }}>{g.unread} new</b></span>}
                  </span>
  ...
  return (
    <div style={{ padding: '6px 12px 10px', display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div className="group-head">
        ...
        <button className="btn small" onClick={() => chatDock.open(group.id)}>💬 Chat</button>
        {isOwner
          ? <button className="btn small danger" onClick={() => void deleteGroup()}>Delete group</button>
          : <button className="btn small" onClick={() => void leave()}>Leave</button>}
      </div>
...
function GroupProjectsTab({ group, can }: { group: GroupSummary; can: (p: GroupPerm) => boolean }) {
  ...
  return (
    <div className="group-tab-body">
      {mine.length === 0 && <p className="muted" style={{ margin: '2px 0 6px' }}>No group projects yet.</p>}
      {mine.map((p) => (
        <div key={p.id} className="row" style={{ paddingLeft: 4 }}>
          ...
          {can('manage_projects') && !p.archived && (
            <button className="icon-btn" aria-label={`Archive ${p.name}`} title="Archive"
              onClick={() => void archiveProject(p)}>📦</button>
          )}
          {can('manage_projects') && (
            <button className="icon-btn" aria-label={`Delete ${p.name}`} title="Delete for all members (no undo)"
              onClick={() => void deleteProject(p)}>🗑</button>
          )}
        </div>
      ))}
      {can('manage_projects') && (
        <div style={{ padding: '6px 0 0' }}>
          <button className="btn small" onClick={() => void createProject()}>＋ Project</button>
        </div>
      )}
    </div>
  );
...
function GroupMembersTab({ group, members, isOwner, can, onReload }: {
  group: GroupSummary; members: MemberRow[] | null; isOwner: boolean;
  can: (p: GroupPerm) => boolean; onReload: () => Promise<void>;
}) {
  ...
  if (members === null) return <div className="muted" style={{ padding: '4px 0' }}>Loading…</div>;
        ...
        return (
          <div key={m.id}>
            <div className="row" role="button" tabIndex={0} aria-expanded={editing === m.id}
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' && isOwner && !self) setEditing(editing === m.id ? null : m.id); }}>
              ...
              {can('remove_members') && !self && m.role !== 'owner' && (
                <button className="icon-btn" aria-label={`Remove ${m.username}`} title="Remove from group"
                  onClick={async (e) => { e.stopPropagation(); await removeMember(m); }}>✕</button>
              )}
            </div>
            {isOwner && !self && editing === m.id && (
              <MemberPermEditor groupId={group.id} member={m} onSaved={() => { setEditing(null); void onReload(); }} />
            )}
          </div>
        );
      })}
...
function GroupInvites({ groupId }: { groupId: string }) {
  ...
  return (
    <div>
      <div className="tree-section-title" style={{ paddingLeft: 0 }}>By username</div>
...
function GroupLinks({ groupId }: { groupId: string }) {
  ...
  return (
    <div>
      <div className="tree-section-title" style={{ paddingLeft: 0 }}>Invite links</div>
