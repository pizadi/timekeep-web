...
import { assertTaskLimit, assertSubtaskLimit, assertNotSubtask, createDependency, assertReparentSafe } from '../rules';
...
taskRoutes.post('/projects/:id/tasks', async (c) => {
  ...
  const now = Date.now();
  const id = ulid(now);
  const posRow = await c.env.DB.prepare('SELECT COALESCE(MAX(position), -1) AS p FROM tasks WHERE project_id = ?1')
    .bind(projectId)
    .first<{ p: number }>();
  await c.env.DB.prepare(
    ...
    .bind(id, c.get('user').id, projectId, parsed.data.name, parsed.data.notes ?? '', (posRow?.p ?? -1) + 1, now)
    .run();
...
taskRoutes.post('/tasks/:id/subtasks', async (c) => {
  ...
  if (parent.parent_id !== null) return jsonError(422, 'two_level_hierarchy', "a subtask can't have subtasks");
  if (access.isGroup && !access.canEditTasks) return jsonError(403, 'forbidden', 'missing permission: edit_tasks');
  ...
  const now = Date.now();
  const id = ulid(now);
  const posRow = await c.env.DB.prepare('SELECT COALESCE(MAX(position), -1) AS p FROM subtasks WHERE task_id = ?1')
    .bind(parent.id)
    .first<{ p: number }>();
  await c.env.DB.prepare(
    ...
    .bind(id, parent.id, userId, parsed.data.name, (posRow?.p ?? -1) + 1, now)
    .run();
...
taskRoutes.patch('/subtasks/:id', async (c) => {
  ...
  if (access.isGroup && !access.canEditTasks) return jsonError(403, 'forbidden', 'missing permission: edit_tasks');
  if (!access.isGroup && row.user_id !== userId) return jsonError(404, 'not_found', 'subtask not found');
...
taskRoutes.delete('/subtasks/:id', async (c) => {
  ...
  const evs = await emitEntityEvents(
    ...
    userId,
    row.project_id,
    [{ type: 'subtask.deleted', actor: c.get('deviceId'), data: { subtask: row } }],
    c.executionCtx,
  );
...
taskRoutes.delete('/tasks/:id/deps/:depId', async (c) => {
  ...
  if (access.isGroup && !access.canEditTasks) return jsonError(403, 'forbidden', 'missing permission: edit_tasks');
  const existing = await c.env.DB.prepare(`SELECT * FROM task_dependencies WHERE task_id = ?1 AND depends_on_id = ?2`)
    .bind(taskId, depId)
    .first();
  if (!existing) return jsonError(404, 'not_found', 'dependency not found');
  await c.env.DB.prepare(`DELETE FROM task_dependencies WHERE task_id = ?1 AND depends_on_id = ?2`)
    .bind(taskId, depId)
    .run();
  const evs = await emitEntityEvents(
    c.env,
