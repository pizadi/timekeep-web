...
reportRoutes.get('/reports/summary', async (c) => {
  ...
  const bucketRows = await c.env.DB.prepare(
    ...
    .bind(daysJson, now, userId, rangeEnd, rangeStart)
  ...
  const donutRows = await c.env.DB.prepare(
    ...
    .bind(now, rangeEnd, rangeStart, userId)
  ...
  const tableRows = await c.env.DB.prepare(
    ...
    .bind(now, todayEnd, todayStart, now, weekStart, userId)
    .all<any>();
  ...
  const table = tableRows.results.reduce<any[]>((acc, r) => {
    ...
    const mins = {
      today: minutes(Number(r.today_ms ?? 0)),
      week: minutes(Number(r.week_ms ?? 0)),
      all: minutes(Number(r.all_ms ?? 0)),
    };
    row.today += mins.today;
    row.week += mins.week;
    row.all += mins.all;
    if (r.subtask_id) {
      row.subtasks.push({
  ...
  return c.json({
    ...
    donut_subtasks: donutSubRows.results.map((r) => ({
      ...
      subtask_name: r.subtask_name ?? null,
...
reportRoutes.get('/reports/heatmap', async (c) => {
  ...
  if (!Number.isInteger(year) || year < 2000 || year > 2200) return jsonError(422, 'validation', 'invalid year');
