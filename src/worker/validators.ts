...
export const adminCreateSchema = z.object({
  username: z
    ...
    .trim()
    .toLowerCase()
    .regex(USERNAME_RE, 'username must be 2–32 chars: lowercase letters, digits, dot, dash, underscore'),
...
export const restoreSchema = z
  ...
  .superRefine((d, ctx) => {
    const total = d.projects.length + d.tasks.length + d.subtasks.length + d.dependencies.length + d.sessions.length;
    if (total > LIMITS.restoreMaxRows) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: `restore payload exceeds ${LIMITS.restoreMaxRows} total rows`,
      });
    }
  });
